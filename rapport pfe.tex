...
\begin{minipage}[t]{0.25\textwidth}
    \vspace{0pt}
    \centering
    \includegraphics[width=3.7cm]{pics/um5_rabat.png}
\end{minipage}
...
\insertfigure[0.78\textwidth]{pics/cycle_vie_bim.png}{Niveaux de maturité BIM}
...
\insertfigure[1.14\textwidth]{pics/architecture_generale_bim_iot2.png}{Architecture générale BIM--IoT}
...
\section{Introduction}
Ce chapitre présente la contribution réalisée sur la modélisation BIM du projet C2R. Cette partie est distincte du développement de RESBIM et de l'intégration BIM--IoT. Elle correspond à un travail de production, d'organisation et de lecture des maquettes disciplinaires, mené dans une logique de coordination conforme aux conventions BIM du projet.

L'objectif est de présenter de manière claire les maquettes modélisées à partir des plans 2D préparés du projet, en distinguant les disciplines principales et leur rôle dans la synthèse BIM.

\begin{samepage}
\section{Présentation du projet C2R}
Le projet C2R constitue le cas support principal de la contribution BIM. Il regroupe plusieurs disciplines : architecture, structure, MEP et synthèse. La maquette MEP est elle-même décomposée en sous-disciplines afin de respecter une organisation claire des responsabilités, des objets modélisés et des contrôles de coordination.

\insertfigure[0.8\textwidth]{pics/vue_3d_c2r2.png}{Vue 3D générale du projet C2R}
\end{samepage}

\section{Organisation générale des maquettes}
Le projet est organisé autour de maquettes disciplinaires liées dans un environnement de synthèse. Cette séparation permet de conserver une lecture claire des différents lots tout en assurant une coordination globale cohérente.

Dans ce cadre, la modélisation a été structurée autour des ensembles suivants :
\begin{itemize}[leftmargin=1.2cm]
    \item une maquette architecture ;
    \item une maquette structure ;
    \item une maquette MEP intégrant les réseaux CFO/CFA, y compris les chemins de câbles, ainsi que les réseaux fluides ;
    \item une maquette de synthèse destinée au regroupement des disciplines.
\end{itemize}

\begin{table}[H]
\centering
\caption{Structuration générale des maquettes du projet C2R}
\begin{tabularx}{0.95\textwidth}{|L{3.6cm}|L{3.6cm}|Y|}
\hline
\textbf{Ensemble} & \textbf{Sous-ensemble} & \textbf{Rôle principal} \\
\hline
Maquette ARC & Architecture & Référence spatiale, organisation des espaces, niveaux, trames, murs, portes et locaux. \\
\hline
Maquette STR & Structure & Supports porteurs, voiles, dalles, poutres, poteaux et éléments traversés par les réseaux. \\
\hline
Maquette MEP & CFO / CFA / Fluides / Synthèse & Modélisation et lecture coordonnée des réseaux techniques, y compris les vues de synthèse dans la même maquette MEP. \\
\hline
\end{tabularx}
\end{table}
...
\hline
MEP -- CFO/CFA & Réseaux électriques de courant fort et courant faible, y compris les chemins de câbles et les équipements associés. \\
\hline
MEP -- Fluides & Réseaux CVC, plomberie, alimentation, évacuation, hydraulique et PCI. \\
\hline
MEP -- Synthèse & Lecture multidisciplinaire et coordination des interfaces dans la même maquette MEP. \\
\hline
\end{tabularx}
\end{table}
...
\section{Maquette architecture (ARC)}
La maquette architecture constitue la référence spatiale du projet. Elle contient les murs, cloisons, portes, fenêtres, niveaux, locaux, circulations et limites générales du bâtiment. Elle a été modélisée à partir des plans 2D préparés du projet afin de disposer d'une base architecturale claire pour les autres disciplines.

Dans le projet C2R, cette maquette sert principalement à :
\begin{itemize}[leftmargin=1.2cm]
    \item définir l'organisation spatiale du bâtiment ;
    \item localiser les zones techniques ;
    \item fournir une base de lecture commune aux maquettes structure et MEP.
\end{itemize}

\insertfigure[0.86\textwidth]{pics/maquette_architecture_c2r.png}{Maquette architecture du projet C2R}

\section{Maquette structure (STR)}
La maquette structure regroupe les éléments porteurs du projet : voiles, dalles, poutres, poteaux, trémies et niveaux structurels. Elle a été modélisée à partir des plans 2D préparés afin de représenter les supports traversés par les réseaux techniques.

Dans le cadre du PFE, cette maquette est particulièrement importante pour :
\begin{itemize}[leftmargin=1.2cm]
    \item identifier les supports réellement traversés par les réseaux ;
    \item qualifier le type de support concerné : voile ou dalle ;
    \item préparer la logique de réservation exploitée ensuite par RESBIM.
\end{itemize}

\insertfigure[0.86\textwidth]{pics/maquette_structure_c2r2.png}{Maquette structure du projet C2R}

\section{Maquette MEP}
La maquette MEP regroupe les réseaux techniques du projet. Dans le cadre de cette contribution, elle est présentée selon deux grands ensembles : les réseaux CFO/CFA, qui incluent également les chemins de câbles, et les réseaux fluides. Cette organisation permet de garder une lecture simple et cohérente de la modélisation réalisée.

\begin{table}[H]
\centering
\caption{Structuration de la maquette MEP dans le projet C2R}
\begin{tabularx}{0.95\textwidth}{|L{3.2cm}|L{4.1cm}|Y|}
\hline
\textbf{Sous-discipline} & \textbf{Contenu principal} & \textbf{Rôle dans la coordination} \\
\hline
CFO/CFA & Réseaux électriques forts et faibles, équipements, câblages et chemins de câbles. & Vérification de l'implantation électrique et de la compatibilité avec les autres lots. \\
\hline
Fluides & Aéraulique, alimentation, évacuation, hydraulique et PCI. & Identification des passages générateurs de réservations circulaires ou rectangulaires. \\
\hline
\end{tabularx}
\end{table}

\subsection{MEP électrique : CFO et CFA}
Les maquettes CFO et CFA couvrent les réseaux électriques du projet. Elles comprennent les équipements, les départs, les terminaux, les réseaux de sécurité, les réseaux de communication ainsi que les chemins de câbles nécessaires au fonctionnement et à la distribution des installations.

Dans la modélisation réalisée, cet ensemble couvre principalement :
\begin{itemize}[leftmargin=1.2cm]
    \item les distributions électriques fortes et faibles ;
    \item les chemins de câbles intégrés aux parcours techniques ;
    \item la lecture des réseaux dans la maquette de synthèse ;
    \item les zones de cohabitation avec la structure et les réseaux fluides.
\end{itemize}
\insertfigure[0.86\textwidth]{pics/reseaux_cfo_cfa_c2r2.png}{Réseaux CFO/CFA du projet C2R}

\subsection{MEP fluides}
La maquette fluides regroupe les réseaux aérauliques, d'alimentation, d'évacuation, hydrauliques et de protection incendie. Elle a été modélisée à partir des plans 2D préparés afin de représenter les réseaux techniques dans leur configuration spatiale et de faciliter leur intégration dans la synthèse.

Dans le cadre du projet C2R, les sous-ensembles suivis sont notamment :
\begin{itemize}[leftmargin=1.2cm]
    \item \textbf{AER} : réseaux aérauliques et gaines de ventilation ;
    \item \textbf{ALIM} : réseaux d'alimentation ;
    \item \textbf{EVAC} : réseaux d'évacuation ;
    \item \textbf{HYD} : réseaux hydrauliques et circuits techniques ;
    \item \textbf{PCI} : protection contre l'incendie.
\end{itemize}

\insertfigure[0.86\textwidth]{pics/reseaux_fluides_c2r2.png}{Réseaux fluides du projet C2R}

\section{Synthèse dans la maquette MEP}
Dans le cadre de cette contribution, la synthèse n'est pas traitée comme une maquette indépendante supplémentaire. Elle correspond à une lecture coordonnée réalisée dans la maquette MEP, afin de visualiser simultanément l'architecture, la structure et les réseaux techniques.

Cette lecture de synthèse permet d'obtenir une vue globale du projet et de visualiser la cohabitation entre architecture, structure, CFO/CFA et fluides dans un même environnement de travail.

\insertfigure[0.92\textwidth]{pics/maquette_synthese_c2r2.png}{Maquette de synthèse du projet C2R}
\insertfigure[0.92\textwidth]{pics/vue_multidisciplinaire_c2r2.png}{Vue multidisciplinaire du projet C2R}
...
\hline
Maquette MEP & CFO/CFA, fluides et synthèse & Coordination des réseaux techniques et lecture globale des interfaces dans la même maquette. \\
\hline
\end{tabularx}
\end{table}

\section{Conclusion}
La contribution à la modélisation C2R constitue une partie séparée et importante du PFE. Elle a permis de modéliser les maquettes architecture et structure, ainsi qu'une maquette MEP intégrant les réseaux CFO/CFA, les fluides et la lecture de synthèse, à partir des plans 2D préparés du projet. Ce travail a fourni une base claire pour la compréhension des interfaces entre disciplines et pour la suite du mémoire consacrée aux réservations structurelles.
